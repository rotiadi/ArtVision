  ...
  async (req, res) => {
    ...
    if (errors.length > 0) {
      ...
      const records = await dataBase
        .query(
          "INSERT INTO cart_products(id_user, id_painting, price, date) VALUES ( $1, $2, $3, $4)",
          [req.id_user, id_painting, price, new Date().toLocaleDateString()]
        )
        .catch((err) => {
          res.status(500);
          res.send({
...
router.post("/view", checkConection, async (req, res) => {
  const records = await dataBase.query(
    `select cp.id, cp.id_user, cp.id_painting, cp.price, cp.date, 
        ...
        where cp.id_user = $1 and cp.status is null`,
    [req.id_user]
  );
